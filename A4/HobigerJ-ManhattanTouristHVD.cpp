...
class Matrix {
    ...
    public:
        ...
        void dimensions_matrix_diagonal(Matrix m) {
            spaltenanz = m.getSpaltenanz()-1;
            zeilenanz = m.getZeilenanz();
            setDimensions();
        }
...
void calculate_node_sums(Matrix nodes, Matrix down, Matrix right, Matrix diagonal);
...
int main(int argc, char* argv[]) { 
    ...
    Matrix diagonal_edges;
    diagonal_edges.dimensions_matrix_diagonal(down_edges);

    fill_matrix(down_edges, cursor, Angabe);
    
    fill_matrix(right_edges, cursor, Angabe);

    fill_matrix(diagonal_edges, cursor, Angabe);
    ...
    calculate_node_sums(knoten, down_edges, right_edges, diagonal_edges);  // hier muss diagonal dazu
}
...
void calculate_node_sums(Matrix nodes, Matrix down, Matrix right, Matrix diagonal) {
    ...
    for (int y = 1; y < nodes.getSpaltenanz(); y++) {
        for (int x = 1; x < nodes.getZeilenanz(); x++) {
            ...
            value3 = nodes.getValue(x-1,y-1) + diagonal.getValue(x-1,y-1);
            max_value_temp = max(value1, value2);
            max_value = max(value3, max_value_temp);
